...
export const signin = async (req, res) => {
  ...
  try {
    ...
    res.status(200).cookie("token", token, { expires: new Date(Date.now() + 604800000), httpOnly: true, sameSite: 'lax', secure: false, }).json({ result: existingUser })
...
export const signup = async (req, res) => {
  ...
  try {
    ...
    res.status(201).cookie("token", token, { expires: new Date(Date.now() + 604800000), httpOnly: true, sameSite: 'none', secure: false, domain: 'localhost' }).json({ result: newUser })
  }
  catch (error) {
    console.log(error);
    res.status(500).json({ message: "Something went wrong. Please Refresh" });
  ...
  if (email_verified) {
    ...
    try {
      if (!existingUser) {
        ...
        res.status(201).cookie("token", token, { expires: new Date(Date.now() + 604800000), httpOnly: true, sameSite: 'none', secure: true, domain: 'localhost' }).json({ result: newUser })
      }
      else {
        const token = jwt.sign({ id: existingUser._id }, process.env.JWT_SECRET, { expiresIn: "7d" });

        res.status(200).cookie("token", token, { expires: new Date(Date.now() + 604800000), httpOnly: true, sameSite: 'none', secure: true, domain: 'localhost' }).json({ result: existingUser })
      }
    } catch (error) {
      console.log(error);
